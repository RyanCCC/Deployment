...
class YOLOP {
public:
    YOLOP(string modelpath, float confThreshold, float nmsThreshold, float objThreshold);
    Mat detect(Mat& frame);
private:
    const float mean[3] = { 0.485, 0.456, 0.406 };
    const float std[3] = { 0.229, 0.224, 0.225 };
    const float anchors[3][6] = { {3, 9, 5, 11, 4, 20}, {7, 18, 6, 39, 12, 31}, {19, 50, 38, 81, 68} };
...
YOLOP::YOLOP(string modelpath, float confThreshold, float nmsThreshold, float objThreshold) {
...
Mat YOLOP::resize_image(Mat srcimg, int* newh, int* neww, int* top, int* left) {
...
void YOLOP::drawPred(int classId, float conf, int left, int top, int right, int bottom, Mat& frame)   // Draw the predicted bounding box
{
